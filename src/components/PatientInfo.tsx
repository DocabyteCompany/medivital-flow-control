    ...
    <Card className="shadow-soft border-0 rounded-2xl w-full lg:w-80 flex-shrink-0">
      <CardContent className="p-6">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="text-xl font-bold text-brand-dark">{t('patientInfo.patientName')}</h2>
          <p className="text-sm text-gray-400">{t('patientInfo.yearsOld', { age: 65, occupation: t('patientInfo.occupation') })}</p>
          <div className="flex items-center bg-green-100 text-status-green text-xs font-medium px-3 py-1 rounded-full mt-2">
            <CheckCircle className="w-3 h-3 mr-1" />
            {t('patientInfo.healthy')}
          </div>
        </div>
  
        <div className="grid grid-cols-3 gap-4 text-center my-6">
          <div>
            <p className="text-xs text-gray-400">{t('patientInfo.blood')}</p>
            <p className="font-bold text-brand-dark">B+</p>
          </div>
          <div>
            <p className="text-xs text-gray-400">{t('patientInfo.height')}</p>
            <p className="font-bold text-brand-dark">170<span className="font-normal text-sm">cm</span></p>
          </div>
          <div>
            <p className="text-xs text-gray-400">{t('patientInfo.weight')}</p>
            <p className="font-bold text-brand-dark">80<span className="font-normal text-sm">kg</span></p>
          </div>
        </div>
  
        <div>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{t('patientInfo.responsibleDoctors')}</h3>
        ...
        <div className="mt-6">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{t('patientInfo.patientHistory')}</h3>
          <button className="flex items-center w-full text-left p-3 bg-gray-50 rounded-lg hover:bg-gray-100">
            ...
            <div>
              <p className="text-sm font-semibold text-brand-dark">{t('patientInfo.patientName')}</p>
              <p className="text-xs text-gray-400">{t('patientInfo.historyFile')}</p>
            </div>
          </button>
        </div>
