...
const DoctorList = ({ selectedConversationId, onSelectConversation }: DoctorListProps) => {
          ...
          <div className="space-y-2">
            {individualConversations.map((conv) => {
              ...
              return (
                <div
                  key={conv.id}
                  onClick={() => onSelectConversation(conv.id, 'individual')}
                  className={cn(
                    "flex items-center p-3 rounded-lg cursor-pointer hover:bg-muted",
          ...
          <div className="space-y-2">
            {groupChannels.map((channel) => {
              ...
              return (
                <div
                  key={channel.id}
                  onClick={() => onSelectConversation(channel.id, 'group')}
                  className={cn(
                    "flex items-center p-3 rounded-lg cursor-pointer hover:bg-muted",
                    selectedConversationId === channel.id && "bg-muted"
                  )}
                >
                  <div className="relative">
