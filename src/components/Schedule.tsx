...
import { appointments, getDoctorForAppointment, Appointment, getPatientForAppointment } from '@/data/appointments';
...
const AgendaAppointmentItem = ({ appointment }: { appointment: Appointment }) => {
  const { t } = useTranslation();
  const doctor = getDoctorForAppointment(appointment.personnelId);
    ...
    <div className="flex items-center justify-between p-4 rounded-lg bg-gray-50 hover:bg-gray-100">
      <div className="flex items-center gap-4 flex-1">
        ...
        <div className="flex-1">
          <p className="font-semibold text-brand-dark">{appointment.type}</p>
          <p className="text-sm text-gray-500">{t('schedule.patient', 'Paciente')}: {patient?.name}</p>
        </div>
      </div>
      <div className="flex items-center gap-2">
        <Avatar className="w-8 h-8">
...
export const Schedule = ({ patient, selectedDate }: ScheduleProps) => {
  const { t } = useTranslation();
  ...
  // Agenda View for when no patient is selected
  ...
  return (
    <div className="space-y-4">
        <h3 className="text-lg font-semibold text-brand-dark">{t('agenda.title', 'Agenda del Día')} - {selectedDate ? selectedDate.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : ''}</h3>
        {dailyAppointments.length > 0 ? (
            <div className="space-y-3">
                {dailyAppointments.map(app => <AgendaAppointmentItem key={app.id} appointment={app} />)}
            </div>
        ) : (
            <div className="text-center py-8 px-4 bg-gray-50 rounded-2xl">
                <p className="font-semibold text-brand-dark">{t('agenda.noAppointments', 'No hay citas para este día.')}</p>
            </div>
        )}
    </div>
  )
